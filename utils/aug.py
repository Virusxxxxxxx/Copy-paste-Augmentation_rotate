...
from general import convert, check_dir, read_label_txt, flip_bbox, rescale_yolo_labels, random_add_patches
...
def copysmallobjects(image_dir, label_dir, save_pic, save_txt, small_img_dir, times, cl_id):
    ...
    save_txt = join(save_txt, dir_name)
    save_pic = join(save_pic, dir_name)
    check_dir(save_txt)
    check_dir(save_pic)
    yolo_txt_dir = join(save_txt, basename(image_dir.replace('.png', '_aug_%s.txt' % str(times))))
    cv2.imwrite(join(save_pic, basename(image_dir).replace('.png', '_aug_%s.png' % str(times))), image)
